...
extension PanelStatsModel {
    ...
    ///   • aggregate — "All accounts ≥95% at once — 3 episodes (1h40m) · swaps 28 · last 7 days"
    ///     (the water is the fixture's own `all_high_threshold`, not a literal — see issue #805;
    ///     likewise its SET is the fixture's own `census_over_roster`, issue #866, and its
    ///     DENOMINATOR the fixture's own `all_high_covered_secs`, issue #1029. All three are sent
    ///     because the CURRENT daemon always sends them, and omitting any would rasterize the
    ///     OLD-daemon branch instead — leaving the live path with no golden. The SET and the
    ///     DENOMINATOR are the halves worth saying out loud: dropping either renders the IDENTICAL
    ///     string here, so no golden would move to betray the omission, whereas dropping the water
    ///     visibly changes the label. The denominator is the FULL window (`604800` = the fixture's
    ///     own `1783200000 → 1783804800`), which is what makes the callout's terse form correct
    ///     rather than merely unannotated: a wholly-measured week carries no coverage clause, and
    ///     that is the state the mock's `healthy-stats-{light,dark}` frames draw.)
    ///
    ...
    ],"summary":{"roster":{"swap_count":28,"swaps":{"session":19,"weekly":5,"manual":3,"forced":1,"emergency":0},"all_high_episodes":3,"all_high_secs":6000,"all_high_threshold":0.95,"all_high_covered_secs":604800,"census_over_roster":true},"accounts":{"Work":{"seen":7,"coverage":1.0,"coverage_class":"complete","session":{"mean":0.42,"peak":1.0,"p95":0.95},"weekly":{"mean":0.6,"peak":0.88,"p95":0.85},"cap_hits":42,"time_at_cap_secs":5400,"contribution_share":0.62,"band":"at_cap"},"Personal":{"seen":7,"coverage":1.0,"coverage_class":"complete","session":{"mean":0.31,"peak":0.96,"p95":0.9},"weekly":{"mean":0.5,"peak":0.71,"p95":0.68},"cap_hits":11,"time_at_cap_secs":900,"contribution_share":0.28,"band":"moderate"},"Temp":{"seen":7,"coverage":1.0,"coverage_class":"complete","session":{"mean":0.04,"peak":0.22,"p95":0.2},"weekly":{"mean":0.1,"peak":0.18,"p95":0.16},"cap_hits":0,"time_at_cap_secs":0,"contribution_share":0.1,"band":"low"}}}}
    """#
}
#endif
