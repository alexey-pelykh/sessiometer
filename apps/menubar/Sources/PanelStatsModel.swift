...
extension PanelStatsModel {
    ...
    ///     likewise its SET is the fixture's own `census_over_roster`, issue #866. Both are sent
    ///     because the CURRENT daemon always sends them, and omitting either would rasterize the
    ///     OLD-daemon branch instead — leaving the live path with no golden. The SET is the half
    ///     worth saying out loud: dropping it renders the IDENTICAL string, so no golden would
    ///     move to betray the omission, whereas dropping the water visibly changes the label.)
    ///
    ...
    ],"summary":{"roster":{"swap_count":28,"swaps":{"session":19,"weekly":5,"manual":3,"forced":1,"emergency":0},"all_high_episodes":3,"all_high_secs":6000,"all_high_threshold":0.95,"census_over_roster":true},"accounts":{"Work":{"seen":7,"coverage":1.0,"coverage_class":"complete","session":{"mean":0.42,"peak":1.0,"p95":0.95},"weekly":{"mean":0.6,"peak":0.88,"p95":0.85},"cap_hits":42,"time_at_cap_secs":5400,"contribution_share":0.62,"band":"at_cap"},"Personal":{"seen":7,"coverage":1.0,"coverage_class":"complete","session":{"mean":0.31,"peak":0.96,"p95":0.9},"weekly":{"mean":0.5,"peak":0.71,"p95":0.68},"cap_hits":11,"time_at_cap_secs":900,"contribution_share":0.28,"band":"moderate"},"Temp":{"seen":7,"coverage":1.0,"coverage_class":"complete","session":{"mean":0.04,"peak":0.22,"p95":0.2},"weekly":{"mean":0.1,"peak":0.18,"p95":0.16},"cap_hits":0,"time_at_cap_secs":0,"contribution_share":0.1,"band":"low"}}}}
    """#
}
#endif
