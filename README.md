...
| `session_trigger` | Swap **away** from the active account at or above this session-usage percent — one predicate on two estimators (reactive `observed`, projected `observed + velocity × H`), **not two knobs**. **Not the landing point**: a correct swap here can still overshoot by a post-swap committed tail (up to +5 pp — in-flight work keeps billing the parked account), and its safe value is *coupled* to `poll_secs` and `session_velocity_horizon_secs`. See [ADR-0022](docs/adr/0022-session-trigger-one-predicate-two-estimators.md). | `50..=99` | `95` |
