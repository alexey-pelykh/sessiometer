...
| `session_trigger` | The session-usage **ceiling** (%) the active account must not cross — *not* a fire-at level. Both swap estimators (reactive `observed`, projected `observed + velocity × H` — one predicate, **not two knobs**) derive their fire point **backward** from it, so the account *lands below* the ceiling even after its post-swap committed tail (up to +5 pp — in-flight work keeps billing the parked account). The default `95` sits below the `P100 < 99` SLO for headroom over re-observation-gap staleness; its safe value stays *coupled* to `poll_secs` and `session_velocity_horizon_secs`. See [ADR-0023](docs/adr/0023-session-trigger-ceiling-semantics.md). | `50..=99` | `95` |
