...
**Rule under test**: duplicate labels are a *documented, accepted* state (`apply_enabled()`'s doc comment in `src/cli.rs` —
...
> `src_config.clone()` (`the_migration_conflict_policy_default_drives_import_behaviour`), so every `account_uuid` matches **by construction** and
...
## Scenario: duplicate-label resolution is consistent across commands  · Cap-3.2
    ...
    But not by settling the three reversible verbs and letting remove inherit the answer

    # Today they do NOT: use refuses with UseTargetAmbiguous (exit 6, resolve_target in src/use_account.rs) while
    # apply_enabled silently takes the earliest entry (apply_enabled in src/cli.rs). Which one is correct
    # is open question OQ-1 and is not settled by this spec.
    #
    # remove is the fourth, and it is the reason the question matters: it resolves a label first-match
    # and DELETES the keychain stash, so it is the only one of the four whose first-match behaviour is
    # irreversible. PRD R-6a's argument is that remove's irreversibility should DRIVE the policy — so a
    # criterion covering only use/enable/disable would be satisfied by exactly the outcome that
    # argument rejects. OQ-1 is stated over all four.
