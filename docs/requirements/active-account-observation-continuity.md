...
| Never-looked-at observability (R-4, R-5) | **NEAR-COMPLETE** | The readout is now **BUILT**; the METER is still **half-satisfiable**, so the row does not move to COMPLETE. **Decided 2026-09-04, delivered 2026-09-06 (issue #1488)**: a `first_sight` block on `ReliabilityWire`, `reliability`'s own `JSON_SCHEMA_VERSION` 12 → 13, additive, mirroring `BlindEpisodesWire`'s enter/exit-pair census — see `docs/design/daemon-diagnostic-integrity-solution-design.md` § 7. What shipped: the four census counts (`n_exit_without_enter`, `n_anchor_lost`, `n_never_recovered`, `n_malformed`), an exit partition (`n` + `n_swapped_away` + `n_not_active`) that is exact by construction, percentiles named `breach_p50` / `breach_p95` / `breach_p100` for the censored population they measure rather than as bare `p50`/`p95` (§ 11 `K-6`), withheld as `null` on an empty subject, and no `targets`/`met` pair — a `GOAL` verdict is not a representable state here. The `FAIL` detector is built at `2T` and reports `n_over_fail_bound_upper`, named for its direction; `T` is the **smallest** `threshold_secs` over the `observation_gap_enter` lines in view, `null` and never assumed when no entry is in view, with `n_entry_thresholds` disclosing a window that straddles a change. Both exit flags parse strictly — anything that is not exactly `true`/`false` is `n_malformed`, never a population fact. No `STATUS_SCHEMA_VERSION` bump, no Swift surface, no status/watch golden — but `reliability`'s human render shares its `Report`, so the three `build/fixtures/cli-renders/reliability-*.txt` renders moved and carry the `CLI-Goldens-Rebaselined:` trailer. Scope bound: the **event-log** readout only — `record_usage_sample` stays inside the `poll_idx` guard, so the usage-sample store still cannot see a never-attempted poll. **Why this stays NEAR-COMPLETE:** `observation_gap_enter` fires only when `elapsed > 2*poll_secs / N` (strictly, and deliberately so), so the emitted set is the breach tail — left-censored at the bound. With `T = 2*poll_secs / N`, the entry edge is `T` while § 6's `FAIL` is *any single occurrence > 2T* — twice that edge — so no occurrence that would trip `FAIL` is missing from the **emitted** set and the censoring alone cannot lose one. It is an **upper bound** rather than an exact count, because the entry anchor is `observed.max(designated)`, so a mid-tenure observation gap on a long-active account also passes the filter while lying outside § 6's post-swap `SCALE`. **A zero is conclusive only conditionally**, and the block says which: `fail_zero_is_conclusive` is `true` only when nothing went unseen, because three populations sit outside the `2T` filter — unplaceable lines never become exits, still-open gaps have no `elapsed_secs` yet, and a severed exit has no `T` of its own. An earlier form of this row asserted the zero was conclusive unconditionally; that was corrected on the same PR, along with a latest-wins `T` that could grade a tighter episode against a looser bound and report a real `FAIL` as absent. It makes the breach tail durable for the first time, which is the value here. What it cannot carry is `GOAL p95 <= T`, a percentile over the **whole** distribution: this set omits everything at or below `T` by construction, so no filtering recovers it and GOAL-met is not a representable state. The daemon's own emitter doc comment in `src/observability.rs` says as much — *"a p50 of first-sight latency needs a source that also records the non-breaches, which no event here is"*. Which source could grade the GOAL half is **OQ-3** in that design's § 11, and it remains open. **Two things now stand between this row and COMPLETE, not one:** `OQ-3`, and the anchor-branch discriminator that design's § 7 left to this item — recorded there on 2026-09-06 as **DEFERRED**, because it changes the daemon's event-emission surface (outside what #1488 owns) and because the SLI reads the *exit* line while the branch is knowable only at *entry*, so consuming it would need the enter↔exit pairing the exit's own doc comment exists to make unnecessary. Until it lands, `n_over_fail_bound_upper` stays an upper bound by construction rather than by censoring. Both are sibling-issue candidates; neither is closable by this row |
