...
> says *"Armed for EVERY outcome, including `Dead` / `Error`"* (`src/daemon/refresh_fold.rs:567`) —
...
|---|---|---|---|
| A-1 | `account_uuid` is stable across machines for one Anthropic account | 🟢 | **Validated** — `src/config.rs:343` ("the Claude `account_uuid`"), sourced from `oauth_account` (`src/capture.rs:463`). This is what makes R-6 a *narrow* case, not the incident's cause (§ 9 F-1) |
