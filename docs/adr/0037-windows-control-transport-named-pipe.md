...
transport **before** the dependent peer-identity/lock item and the run-loop work are built on it.
...
- **The transport cluster dominates the rest** — `UnixStream` and `UnixListener` account for most
  of the remaining diagnostics, joined by `std::os::fd` and `tokio::signal::unix`. A per-symbol
  count is deliberately **not** quoted: one source line can raise several diagnostics and one
  diagnostic can name several symbols, so any such number measures the compiler's grouping rather
  than the porting surface, and it would be stale on landing besides. The `from_mode` /
  `PermissionsExt` diagnostics in the same output are a **different** cluster (Unix file modes) and
...
- **The single-instance guarantee gains a kernel-enforced half.** `first_pipe_instance` denies a
  second create against a held name outright, measured.
...
Recorded as residuals for the dependent implementation item rather than left to be rediscovered:
