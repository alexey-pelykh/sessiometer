...
| `config validate` | Stop reporting a zero-account config as unqualifiedly valid (R-11). Today `render_config_validate` emits `"{path} is valid (0 accounts)"` at exit 0 (`src/cli.rs:2181-2189`); `require_roster`'s three call sites are `src/use_account.rs:988`, `src/poke.rs:174` and the daemon `run` startup gate at `src/cli.rs:1389`, none of them here | none |
