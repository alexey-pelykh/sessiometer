...
// capture / migration) and for the in-module test suite, and needs none of the re-export blocks
// `daemon` carries. `config` retains the type schema itself, the derived accessors, and the
// tests.
//
...
// daemon-routed `config-get` / `config-set` backend. Declared alphabetically below because
// `rustfmt` sorts `mod` statements, so the lifecycle order lives here.
//
...
// `basic_string` belongs to the emitter (`render`), but one parent test pins its full escape
// surface. A PRIVATE re-import — `pub(super) use` would be E0364 against the child's
// `pub(super)` item — cfg(test)-gated so a non-test build sees no unused import.
#[cfg(test)]
use render::basic_string;
