...
mod tests {
    ...
    // copy never grew.
    //
    // Neither message exists as a literal in `src/`, which is why a grep-shaped gate cannot reach
    // either one: the fleet band is `format!`-interpolated from
    // `crate::stats::FLEET_RUNWAY_PLAUSIBLE_MAX_SECS`, and the `unknown field` enumeration is
    // generated by serde's derive from `SetTunables`' declaration. Both are free at RUNTIME, so
    ...
    // `ConfigSetRejectDetailParityTests` reddens until `Fixtures.swift` follows.

    /// The two `config-set` rejection `detail` strings the Swift fixtures copy verbatim, each
    /// field named for the fixture that copies it. The Swift consumer decodes this same JSON with
    /// `.convertFromSnakeCase`, so these field names are the wire keys.
    ...
    /// Produce both details from the code that authors them, as of this build.
    ...
    /// `SetTunables`' key set or to `Config::validate`'s fleet-runway message:
    ///   `cargo test -- --ignored emit_config_set_reject_details_golden`
    /// then update the two Swift fixtures (`apps/menubar/Tests/Fixtures.swift`) so the parity
    /// gate holds.
    #[test]
    ...
    fn the_committed_config_set_reject_details_still_match_this_crate() {
        ...
        // Non-degeneracy. A golden holding two empty strings would satisfy the equalities below
